...
### Fixed
- **Windows Release Workflow Shell Fix**: Added `shell: cmd` and carets (`^`) for Windows multi-line PyInstaller execution step in `.github/workflows/release.yml`, resolving PowerShell `ParserError: Missing expression after unary operator '--'`.
